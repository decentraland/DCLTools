...
class OBJECT_OT_clean_unused_materials(bpy.types.Operator):
    ...
    bl_description = (
        "Remove unused materials: slots not referenced by any face, "
        "and/or globally orphan materials with zero users"
    )
    ...
    clean_unused_slots: bpy.props.BoolProperty(
        ...
        description=(
            "Remove material slots that are assigned to an object but "
            "not used by any face in the mesh"
        ),
        default=True,
    )
    ...
    def _get_unused_slots(self):
        ...
        unused = []
        objects = (
            bpy.context.selected_objects if self.scope_selected
    ...
    def execute(self, context):
        ...
        if self.clean_unused_slots:
            objects = (
                context.selected_objects if self.scope_selected
                else list(bpy.data.objects)
            )
    ...
    def draw(self, context):
        ...
        if self.clean_unused_slots:
            ...
            if unused_slots:
                ...
                # Group by object
                by_obj = {}
                for obj, idx, mat_name in unused_slots:
